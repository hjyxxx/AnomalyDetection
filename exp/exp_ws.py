...
class ExpWS(ExpBasic):
    def __init__(self, args):
        super(ExpWS, self).__init__(args)
    ...
    def vali(self, dataset, dataloader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_rec_x, batch_rec_y) in enumerate(dataloader):
                ...
                batch_scores = torch.mean(torch.nn.functional.mse_loss(preds, trues, reduction='none'), dim=(-1, -2, -3))

                scores.append(batch_scores.unsqueeze(-1))
        ...
        return total_loss, total_preds, total_trues, scores

    def train(self):
        # 获取数据集
        pre_train_dataset, pre_train_loader = self._get_data(flag='pre_train')
        ...
        pre_train_len = len(pre_train_dataset) if pre_train_dataset is not None else 0
        ...
        cprint.info('pre_train: {}'.format(pre_train_len))
        ...
        # 预训练
        cprint.info("**********************start pre_train**********************")
        for epoch in range(5):
            cprint.info("=" * 90)
            cprint.info(f"| Epoch {epoch} start pre_train |")
            pre_train_loss = []     # 保存训练损失
            self.model.train()
            ep_start_time = time.time()
            for batch_id, (batch_rec_x, batch_rec_y) in enumerate(pre_train_loader):
                batch_rec_x = batch_rec_x.float().to(self.device)
                batch_rec_y = batch_rec_y.float().to(self.device)
                ...
                cprint.info("| Pre_Train | Epoch {:3d} | {:5d}/{:5d} batches | lr {:02.5f} | ms/batches {:5.2f} | loss {:5.8f} |".format(
                    epoch, batch_id, len(pre_train_loader), optimizer.param_groups[0]['lr'], bt_elapsed, loss.item()
                ))
        ...
        for epoch in range(1, self.args.train_epochs + 1):
            ...
            self.model.train()
            ep_start_time = time.time()
            for batch_id, (batch_rec_x, batch_rec_y) in enumerate(train_loader):
                batch_rec_x = batch_rec_x.float().to(self.device)
                batch_rec_y = batch_rec_y.float().to(self.device)
            ...
            cprint.info("**********************Change Label Start**********************")
            # 经过一轮训练更改训练集标签值
            _, _, _, train_scores = self.vali(dataset=train_dataset, dataloader=train_loader, criterion=criterion)
            train_dataset.reset_seg_label(train_scores)
            cprint.info("**********************Change Label End**********************")
            ...
            if vali_len != 0:
                ...
                vali_loss, vali_preds, vali_trues, _ = self.vali(vali_dataset, vali_loader, criterion)
            ...
            if test_len != 0:
                ...
                test_loss, test_preds, test_trues, _ = self.vali(test_dataset, test_loader, criterion)
